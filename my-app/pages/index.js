...
export default function Home() {
  ...
  const _swapTokens = async () => {
    try {
      ...
      // We are here using 'eq' (equals) method from BigNumber class in `ethers.js`.
      if (!swapAmountWei.eq(zero)) {
        const signer = await getProviderOrSigner(true);
        setLoading(true);
        // Call the swapTokens function from the `swap.js` file in `utils` folder
        await swapTokens(
          signer,
          swapAmountWei,
  ...
  const _addLiquidity = async () => {
    try {
      ...
      if (!addCDTokens.eq(zero) && !addEtherWei.eq(zero)) {
        const signer = await getProviderOrSigner(true);
        setLoading(true);
        //call the addLiquidity function from the `addLiquidity.js` file in `utils` folder
        await addLiquidity(signer, addCDTokens, addEtherWei);
        setLoading(false);
        //Reinitialize the CD tokens
        setAddCDTokens(zero);
        // Ger amounts for all values after the liquidity has been addedValue
        await getAmounts();
      } else {
        setAddCDTokens(zero);
      }
  ...
  const _getTokensAfterRemove = async (_removeLPTokens) => {
    try {
      ...
      // Get the Eth reserves from the Exchange contract.
      const _ethBalance = await getEtherBalance(provider, null, true);
  ...
  return (
    <div>
      ...
      <div className={styles.main}>
        ...
        <div>
          <img className={styles.image} src="./cryptodev.svg" alt="" />
        </div>
      </div>
